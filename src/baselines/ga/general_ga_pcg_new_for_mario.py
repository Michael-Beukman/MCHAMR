...
class _GeneralGAIndividualMario(_GeneralGAIndividualMaze):
    """
        A single individual for the Mario Game.
    """
    ...
    def crossover(self, other: "_GeneralGAIndividual") -> Tuple["_GeneralGAIndividual", "_GeneralGAIndividual"]:
        ...
        for i, c in enumerate(children):
            ...
            c[i2:] = parents[i][i2:]
        children = [
            _GeneralGAIndividualMario(MarioLevel.from_map(
                                             map=vec.reshape(self.level.height, self.level.width)))
            for vec in children
        ]
        return children
...
class GeneralGAPCG(PCGMethod):
    ...
    def evaluate(self) -> List[float]:
        ...
        if total_fit == 0: # distribute equal fitness to all indivs if all have 0 fitness.
            e = 1 / len(probs)
            probs = [p + e for p in probs]
        return probs
...
def _test_mario():
    ...
    alg = GeneralGAPCG(Game(level), level, lambda l: _GeneralGAIndividualMario(l), fitness_function=fit, population_size=50)
    for i in range(5):
        alg.one_gen()
        print(i, alg.best_fit)
