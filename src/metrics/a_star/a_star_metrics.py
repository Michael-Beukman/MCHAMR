...
class AStarDiversityAndDifficultyMetric(Metric):
    ...
    def __init__(self, game: Game, use_edit_distance=False, n_samples: Union[int, None] = None, 
        ...
        self.all_path_lengths = []
        self.all_count_walls = []
    ...
    def evaluate(self, levels: List[Level], action_trajectories: Union[None, AllActionsAndTrajectories] = None) -> List[float]:
        if self.number_of_times_to_do_evaluation > 1:
            ...
            for i in range(self.number_of_times_to_do_evaluation):
                ...
                self.all_path_lengths.append(self.path_lengths)
                self.all_count_walls.append(self.count_walls)
            ...
            self.path_lengths  = np.amax(self.all_path_lengths, axis=0)
            self.count_walls  = np.amax(self.all_count_walls, axis=0)
    ...
    def _get_action_trajectories(self, levels: List[Level]) -> AllActionsAndTrajectories:
        ...
        if isinstance(self.game, MarioGame):
            ...
            for level in levels:
                ...
                number_of_walls = (level.map == 1).sum()
                count_walls.append(number_of_walls)
...
class AStarPathLengthMetric(Metric):
    """Path Length Metric
    """
    ...
    def evaluate(self, levels: List[Level]) -> List[float]:
        if len(self.parent.path_lengths) == 0:
            self.parent.evaluate(levels)
        return self.parent.path_lengths
    
    def name(self):
        return "PathLength"

class AStarWallCountsMetric(Metric):
    """Path Length Metric
    """
    ...
    def evaluate(self, levels: List[Level]) -> List[float]:
        if len(self.parent.count_walls) == 0:
            self.parent.evaluate(levels)
        return self.parent.count_walls
