...
NoveltyFunction = Callable[[np.ndarray, np.ndarray], float]
NoveltyDistanceFunction = NoveltyFunction
...
def get_only_reachable_from_start_correct(l: np.ndarray) -> np.ndarray:
    """Returns an np.array from an existing one where the tiles reachable from the start are 1 and the rest are 0s
    ...
    labelled = morph.label(l + 1, connectivity=1)
    new = np.zeros_like(l)
    new[labelled != labelled[0, 0]] = 0
    new[labelled == labelled[0, 0]] = 1
    return new
...
def visual_diversity_only_reachable_normalised_version_2(a: np.ndarray, b: np.ndarray):
    """This performs the same visual diversity as above, but first masks out all of the tiles that aren't reachable from the start. This is normalised and uses get_only_reachable_from_start_correct

    Args:
        a (np.ndarray): The levels to compare
        b (np.ndarray): The levels to compare
    """
    a = get_only_reachable_from_start_correct(a)
    b = get_only_reachable_from_start_correct(b)
    return visual_diversity_normalised(a, b)
...
def dist_jensen_shannon_compare_probabilities_add_epsilon(a: np.ndarray, b: np.ndarray, eps: float = 0) -> float:
    """This does the following:
        Create a probability distribution for each level, consisting of how many tiles are 1s and 0s and computes
        the JS divergence between this
    ...
    Returns:
        float: The distance. Between 0 and 1.
    """
    poss = list(set(np.unique(a)) | set(np.unique(b)))
    
    p1 = _get_prob_dist_from_array(a, possibles=poss) + eps
    p2 = _get_prob_dist_from_array(b, possibles=poss) + eps
    return scipy.spatial.distance.jensenshannon(p1, p2, base=2)

def dist_jensen_shannon_compare_probabilities_eps1e4(a: np.ndarray, b: np.ndarray) -> float:
    return dist_jensen_shannon_compare_probabilities_add_epsilon(a, b, 1e-4)
...
def _general_image_hash_distance(which_hash: Callable[[Image.Image, int], imagehash.ImageHash], a: np.ndarray, b: np.ndarray,
    ...
    """
    if len(a.shape) == 3 and a.shape[1] == 1: a = a[:, 0]
    if len(b.shape) == 3 and b.shape[1] == 1: b = b[:, 0]
...
def minecraft_js_divergence_tile_distros_good_6(a, b):
...
def minecraft_js_divergence_tile_distros_5(A: np.ndarray, B: np.ndarray):
    N = 5
    prob_a = []
    prob_b = []
    for i in range(N):
