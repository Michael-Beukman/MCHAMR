...
class NumberOfWallFitness(NeatFitnessFunction):
    ...
    def calc_fitness(self, nets: List[LevelNeuralNet], levels: List[List[Level]]) -> List[float]:
        ...
        for group in levels:
            lilans = 0 
            for level in group:
                lilans += level.map.mean()
            lilans /= len(group)
            ans.append(lilans)
        return ans
    def __repr__(self) -> str:
        return f"NumberOfWallFitness(number_of_levels_to_generate={self.number_of_levels}, level_gen={self.level_gen})"
    
    
class BetterMazeSolvabilityFitness(NeatFitnessFunction):
    """This is a better solvability fitness, that actually checks the top left cell.
    """
    def __init__(self, number_of_levels_to_generate: int, level_gen: NeatLevelGenerator):
        super().__init__(number_of_levels_to_generate=number_of_levels_to_generate, level_gen=level_gen)
    ...
    def calc_fitness(self, nets: List[LevelNeuralNet], levels: List[List[Level]]) -> List[float]:
        ...
        for level_group in levels:
            ...
            for l in level_group:
                ...
                if connected[0, 0] == connected[-1, -1] and l.tile_types[int(l.map[0, 0])] == 'empty':
                    ans += 1
            answer.append(ans / len(level_group))
        return answer
