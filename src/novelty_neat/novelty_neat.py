    ...
    def train(self, logger: Logger) -> List[Dict[str, Any]]:
        ...
        def fitness(genomes: List[Tuple[int, neat.DefaultGenome]], config: neat.Config):
            ...
            for fit, (_, genome) in zip(all_fitnesses, genomes):
                ...
                idx += 1
            if logger is not None and logger.SAVE_GENOMES and hasattr(self.fitness_calculator, 'fitness_components'):
                # Log some data about the current generation, the best/worst genomes and their fitnesses
                d = {'all': []}
                new_genes = []
                for idx in [min_idx, max_idx]:
                    l = self.fitness_calculator.fitness_components[idx]
                    for k, v in l.items():
                        if k not in d: d[k] = []
                        d[k].append(v)
                    d['all'].append(all_fitnesses[idx])
                    new_genes.append(genomes[idx])
                # get means
                total = {'all': 0}
                for idx in range(len(self.fitness_calculator.fitness_components)):
                    l = self.fitness_calculator.fitness_components[idx]
                    for k, v in l.items():
                        if k not in total: total[k] = 0
                        total[k] += v
                    total['all'] += all_fitnesses[idx]
                
                all_genomes_fitnesses.append({
                    'fitness': d,
                    'genomes': new_genes,
                    'total': total,
                    'pop_size': len(self.fitness_calculator.fitness_components)
                })
            # Log some info.
            ...
            if logger.verbose == Verbosity.PROGRESS:
                print(f"\r{steps} / {self.num_generations}: Max fit {np.round(np.max(all_fitnesses), 3)}", end='')
        ...
        if logger.verbose == Verbosity.PROGRESS:
            print("")
        return [{'final_agent': self.best_agent, 'population': self.pop, 'all_genomes_fitnesses': all_genomes_fitnesses}]
