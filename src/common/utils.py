...
def mysavefig(*args, do_tight_layout: bool=True, save_pdf: bool = False, **kwargs):
    if do_tight_layout:
        plt.tight_layout()
    ...
    args = list(args)
    if 'dpi' not in kwargs: kwargs['dpi'] = 100
    plt.savefig(*args, **kwargs)
    if save_pdf:
        args[0] = args[0].split(".jpg")[0] + ".pdf"
        plt.savefig(*args, **kwargs)
...
def remove_axis_ticks_keep_border(ax):
    ax.tick_params(
...
def mysubplots_directly(n_things: int, basefigsize: float = 5, force_size=None, ravel=True, *args, **kwargs):
...
if __name__ == '__main__':
    F = load_compressed_pickle('../results/experiments/pcgnn_3550-cfd/Minecraft2DTownLevel30Size/PCGNN/2022-11-03_12-27-06/50/150/0/seed_0_name_pcgnn_3550-cfd_2022-11-03_16-30-31.p.pbz2')
    s = F['train_results'][0]['all_genomes_fitnesses']
    print(len(s))
    print((s[0]))
    print((s[-1]))